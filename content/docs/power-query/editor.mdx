...
See [Error Handling in Power Query](/docs/power-query/error-handling) for the full pattern — including `try` without `otherwise`, checking `[HasError]`, and when catching the error is the wrong fix.
