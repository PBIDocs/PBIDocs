...
- [EARLIER](/docs/dax/earlier) — reaching back to an outer row context when a calculated column nests a second one.

Wrapped a column reference in `CALCULATE()` inside `FILTER()` and got a "True/False expression" error? Row context already gives direct column access — see [that error explained](/blog/calculate-true-false-expression-error).
