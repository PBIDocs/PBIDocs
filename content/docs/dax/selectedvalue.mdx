...
Using `VALUES()` directly instead and hitting "a table of multiple values was supplied where a single value was expected"? See [that error explained](/blog/table-of-multiple-values-supplied-error) — it's exactly the gap SELECTEDVALUE() is built to close.
