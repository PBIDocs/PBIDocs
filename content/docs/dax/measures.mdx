...
See it applied end to end: [Build a Complete Sales Analysis Report](/tutorials/build-a-sales-analysis-report) writes a real set of measures on top of a real model, start to finish, or [Build an Earned Value Management Dashboard](/tutorials/build-an-evm-dashboard) for a program-management measure set (CPI, SPI, EAC) built the same way.
