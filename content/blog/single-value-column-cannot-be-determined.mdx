...
Getting "a table of multiple values was supplied where a single value was expected" instead? That's a related but distinct error — a table-returning function like `VALUES()` or `FILTER()`, not a bare column. See [that error explained](/blog/table-of-multiple-values-supplied-error).
