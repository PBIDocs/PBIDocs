...
faq:
  ...
  - question: "Why do some bad dates fail loudly while others just parse wrong with no error?"
    answer: "A date like 13/04/2026 only makes sense one way (day 13, month 4) — a month-first locale like en-US rejects it outright since there's no 13th month, while a day-first locale like en-GB parses it fine as April 13th. A date like 03/04/2026 is valid as either March 4th or April 3rd under either locale, so a locale mismatch parses it \"successfully\" into the wrong date with no error at all."
...
Try a plain integer like `1234` — no separators at all means no ambiguity, so both locales agree. Then try `1,234` on its own — even a single comma is ambiguous between "one thousand two hundred thirty-four" and "one point two three four" depending on locale.
...
`13/04/2026` is actually the easier case: no month goes past 12, so a month-first locale like `en-US` rejects it outright — 13 isn't a valid month, and the conversion errors loudly instead of guessing. A day-first locale like `en-GB` parses the same text fine, as April 13th. Either way, there's a clear signal something needs checking. The genuinely dangerous version is a date like `03/04/2026`, which is valid in *both* `DD/MM/YYYY` and `MM/DD/YYYY` — one means March 4th, the other April 3rd. If the locale assumption is wrong, this parses **successfully into the wrong date**, with no error at all.
...
Try `13/04/2026` — only one locale can parse it, and the other fails loudly instead of guessing. Then try `03/04/2026` again to see the silent, both-succeed case.
